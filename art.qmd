...
```{r}
#| label: fig-voronoi
#| fig-cap: Simple Voronoi Diagram
...
#| label: fig-raster
#| fig-cap: 
#|   - Fill is sequential from 1 to $n^2$
#|   - Fill is sum $x + y$
#|   - Fill is product $x \times y$
#|   - Fill is quotient $x / y$
#|   - Fill is permuted sequence from 1 to $n^2$
...
## Raster with Distribution fill

```{r raster2}
#| warning: false
#| fig-asp: 1
#| cache: true
#| #| label: fig-rasterdist
#| fig-cap: 
...
### Raster with Distribution fill2

```{r raster3}
#| warning: false
#| fig-asp: 1
#| cache: true
#| #| label: fig-rasterdist2
#| fig-cap: 
#|   - Fill is $\text{beta}(x,y)$
#|   - Fill is $\text{weibull}(x,y)$
#|   - Fill is $N(x, y^2)$
...
draw2 <- \(n = 1001, fill = 1:n^2, interpolate = TRUE) {
...
c(
  quote(rbeta(n^2, abs(x), abs(y))),
  quote(rweibull(n^2, abs(x), abs(y))),
  quote(rnorm(n^2, x, y^2))
) |>
  future_map(.f = \(fi) draw2(fill = fi, interpolate = FALSE))
```
...
```{r}
#| label: fig-bubbles
#| fig-cap: Randomly placed, colored, and sized bubbles
...
#| fig-asp: 1
#| label: fig-circles
#| fig-cap: Paired overlapping circles
